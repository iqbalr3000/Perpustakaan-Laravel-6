    ...
    <form action="{{ route('siswas.update',$siswa->id) }}" method="POST">
        ...
        @method('PUT')
         ...
         <div class="col-xs-12 col-sm-12 col-md-12">
            ...
            <div class="form-group">
                <strong>Rayon</strong>
                <select class="form-control" name="nama_rayon" id="nama_rayon">
                @foreach($rayons as $rayon)
               
                <option value ="{{$rayon->nama_rayon}}"@if($rayon->nama_rayon == $siswa->nama_rayon)
                    selected @endif >{{$rayon->nama_rayon}} </option>
